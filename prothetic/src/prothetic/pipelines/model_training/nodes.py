...
def train_model(X_train: pd.DataFrame, y_train: pd.DataFrame, X_test: pd.DataFrame, y_test: pd.DataFrame):
    ...
    y_train_reshaped = y_train.values.reshape(
        (y_train.shape[0], y_train.shape[1], 1))
    y_test_reshaped = y_test.values.reshape(
        (y_test.shape[0], y_test.shape[1], 1))
    ...
    model.fit(X_train_reshaped, y_train_reshaped, epochs=10, validation_data=(
        X_test_reshaped, y_test_reshaped), callbacks=[early_stopping])
